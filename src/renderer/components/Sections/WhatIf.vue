  ...
  <div>
    <nav class="level">
      ...
      <div class="level-right">
        <div class="level-item">
          <button class="button is-danger is-small" @click="$router.go(-1)">Exit w/o Saving</button>
        </div>
        <div class="level-item">
          <button class="button is-primary is-small" @click="flushChangeQueue()">Save &amp; Exit</button>
        </div>
      </div>
    </nav>
    <section>
      <b-tabs v-model="activeTab">
          ...
          >
            <template slot-scope="props">
              ...
              <b-table-column
                ...
                sortable
              >{{ props.row.acName }}</b-table-column>
              <b-table-column
                field="totalPoints"
                label="Total Points"
                sortable
              >{{ props.row.totalPoints }}</b-table-column>
              <b-table-column
                field="dateCreated"
                label="Date Created"
                sortable
              >{{ new Date(props.row.dateCreated).toLocaleDateString() }}</b-table-column>
              <b-table-column
                field="dueDate"
                label="Due Date"
                sortable
              >{{ new Date(props.row.dueDate).toLocaleDateString() }}</b-table-column>
        ...
        <b-tab-item label="Section Settings">
          ...
          <nav class="level" v-if="!loading">
            ...
            <div class="level-item">C: &lt;{{section.gradeScaleB}} - {{section.gradeScaleC}}</div>
            <div class="level-item">D: &lt;{{section.gradeScaleC}} - {{section.gradeScaleD}}</div>
            <div class="level-item">F: &lt;{{section.gradeScaleD}} - 0</div>
          </nav>
          <crud-modal-bar
          ...
          >
            <template slot-scope="props">
              <b-table-column field="Name" label="Name" width="300" sortable>{{ props.row.name }}</b-table-column>
              <b-table-column field="Weight" label="Weight" sortable>{{ props.row.weight }}</b-table-column>
              <b-table-column
                field="Lowest Grades Dropped"
                label="Lowest Grades Dropped"
                sortable
              >{{ props.row.lowestGradesDropped }}</b-table-column>
              <b-table-column>
                <crud-modal-bar
                  @click="selectedAssignment = props.row"
                  @acedit="updateAC($event)"
...
export default {
  ...
  data() {
    const data = {
      ...
      GradeScaleInputs: {
        ...
        preCreate: null,
        // postCreate(result) {
        //   EventBus.$emit('section-added', result);
        // },
        // postUpdate(result) {
        //   EventBus.$emit('section-updated', result);
        // },
        // postDelete(result) {
        //   EventBus.$emit('section-removed', result);
        // },
        templates: {
          // courseId: {
          //   label: 'Course',
  ...
  methods: {
    ...
    updateAC(AC) {
      this.assCats.splice(this.assCats.findIndex(ac => ac.id === AC.id), 1, AC);
      if (AC.new) {
    ...
    deleteAC(AC) {
      if (this.assignments.find(a => a.assignmentCategoryId === AC.id)) {
        this.assignments
          .filter(a => a.assignmentCategoryId === AC.id)
          .forEach(a => {
            this.grades
      ...
      const rows = this.sectionStructure.students.map(realStudent => {
        ...
        student.assCats.forEach(ac => {
          ...
          if (
            ...
            let actualTotal = 0;
            for (
              let i = ac.lowestGradesDropped;
              i < ac.assignments.length;
              i += 1
            ) {
